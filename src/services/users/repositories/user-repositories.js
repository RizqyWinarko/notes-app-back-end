...
class UserRepositories {
  constructor() {
    // Perbaikan 1: Tambahkan underscore agar sama dengan yang di bawah
    this._pool = new Pool();
  }

  async createUser({ username, password, fullname }) {
    const id = `user-${nanoid(16)}`; // Opsional: Tambahkan prefix 'user-' agar rapi
  ...
  async getUserById(id) {
    const query = {
      text: 'SELECT id, username, fullname FROM users WHERE id = $1', // <-- Hanya ambil 3 ini
      values: [id],
    };
